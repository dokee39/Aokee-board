...
#ifndef MODM_STM32_SPI_MASTER1_DMA_HPP
#	error 	"Don't include this file directly, use 'spi_master_1_dma.hpp' instead!"
#endif
...
void
modm::platform::SpiMaster1_Dma<DmaChannelRx, DmaChannelTx>::initialize()
{
	// Configure the DMA channels, then calls SpiMaster1::initialzie().
	...
	Dma::RxChannel::setPeripheralAddress(SPI1_BASE + 0x0c);
	...
	Dma::TxChannel::configure(DmaBase::DataTransferDirection::MemoryToPeripheral,
			...
			DmaBase::MemoryIncrementMode::Increment, DmaBase::PeripheralIncrementMode::Fixed,
			DmaBase::Priority::High);
	Dma::TxChannel::setPeripheralAddress(SPI1_BASE + 0x0c);
...
modm::platform::SpiMaster1_Dma<DmaChannelRx, DmaChannelTx>::transfer(uint8_t data)
{
	...
	if ( !(state & Bit0) )
	{
		...
		SpiHal1::disableInterrupt(SpiBase::Interrupt::TxDmaEnable |
		...
		const uint8_t *tx, uint8_t *rx, std::size_t length)
{
	...
	case 0:
		...
		state |= Bit1;
		dmaError = false;
		SpiHal1::enableInterrupt(SpiBase::Interrupt::TxDmaEnable |
				SpiBase::Interrupt::RxDmaEnable);
...
template <class DmaChannelRx, class DmaChannelTx>
void
modm::platform::SpiMaster1_Dma<DmaChannelRx, DmaChannelTx>::handleDmaTransferError()
{
	SpiHal1::disableInterrupt(SpiBase::Interrupt::TxDmaEnable |
			SpiBase::Interrupt::RxDmaEnable);
	Dma::RxChannel::stop();
...
template <class DmaChannelRx, class DmaChannelTx>
void
modm::platform::SpiMaster1_Dma<DmaChannelRx, DmaChannelTx>::handleDmaReceiveComplete()
{
	Dma::RxChannel::stop();
...
template <class DmaChannelRx, class DmaChannelTx>
void
modm::platform::SpiMaster1_Dma<DmaChannelRx, DmaChannelTx>::handleDmaTransmitComplete()
{
	Dma::TxChannel::stop();
