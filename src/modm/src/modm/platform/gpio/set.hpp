...
public:
	static void setOutput()
	{
		...
		if constexpr (mask(2)) GPIOF->MODER = (GPIOF->MODER & ~mask2(2)) | mask2(2, i(Mode::Output));
		if constexpr (mask(3)) GPIOG->MODER = (GPIOG->MODER & ~mask2(3)) | mask2(3, i(Mode::Output));
	}
	...
	static void configure(OutputType type, OutputSpeed speed = OutputSpeed::MHz50)
	{
		...
		if constexpr (mask(2)) {
			GPIOF->OSPEEDR = (GPIOF->OSPEEDR & ~mask2(2)) | (i(speed) * mask2(2, 0b01));
			GPIOF->OTYPER  = (GPIOF->OTYPER  & ~mask(2))  | (i(type) ? mask(2) : 0);
		}
		if constexpr (mask(3)) {
			GPIOG->OSPEEDR = (GPIOG->OSPEEDR & ~mask2(3)) | (i(speed) * mask2(3, 0b01));
			GPIOG->OTYPER  = (GPIOG->OTYPER  & ~mask(3))  | (i(type) ? mask(3) : 0);
		}
	}
	...
	static void setInput()
	{
		...
		if constexpr (mask(2)) {
			GPIOF->MODER &= ~mask2(2);
			GPIOF->OTYPER &= ~mask(2);
			GPIOF->OSPEEDR &= ~mask2(2);
		}
		if constexpr (mask(3)) {
			GPIOG->MODER &= ~mask2(3);
			GPIOG->OTYPER &= ~mask(3);
			GPIOG->OSPEEDR &= ~mask2(3);
		}
	}
	...
	static void setAnalogInput()
	{
		...
		if constexpr (mask(2)) GPIOF->MODER |= mask2(2, i(Mode::Analog));
		if constexpr (mask(3)) GPIOG->MODER |= mask2(3, i(Mode::Analog));
	}
	...
	static void configure(InputType type)
	{
		...
		if constexpr (mask(2)) {
			GPIOF->PUPDR = (GPIOF->PUPDR & ~mask2(2)) | (i(type) * mask2(2, 0b01));
		}
		if constexpr (mask(3)) {
			GPIOG->PUPDR = (GPIOG->PUPDR & ~mask2(3)) | (i(type) * mask2(3, 0b01));
		}
	}
	...
	static void set()
	{
		...
		if constexpr (mask(2)) GPIOF->BSRR = (inverted(2) << 16) | (mask(2) & ~inverted(2));
		if constexpr (mask(3)) GPIOG->BSRR = (inverted(3) << 16) | (mask(3) & ~inverted(3));
	}
	...
	static void reset()
	{
		...
		if constexpr (mask(2)) GPIOF->BSRR = ((uint32_t(mask(2)) & ~inverted(2)) << 16) | inverted(2);
		if constexpr (mask(3)) GPIOG->BSRR = ((uint32_t(mask(3)) & ~inverted(3)) << 16) | inverted(3);
	}
	...
	static void toggle()
	{
		...
		if constexpr (mask(2)) {
			const uint32_t are_set = (GPIOF->ODR & mask(2));
			const uint32_t are_reset = mask(2) ^ are_set;
			GPIOF->BSRR = (are_set << 16) | are_reset;
		}
		if constexpr (mask(3)) {
			const uint32_t are_set = (GPIOG->ODR & mask(3));
	...
	static void lock()
	{
		...
		if constexpr (mask(2)) {
			GPIOF->LCKR = 0x10000 | mask(2);
			GPIOF->LCKR = 0x00000 | mask(2);
			GPIOF->LCKR = 0x10000 | mask(2);
			(void) GPIOF->LCKR;
		}
		if constexpr (mask(3)) {
			GPIOG->LCKR = 0x10000 | mask(3);
			GPIOG->LCKR = 0x00000 | mask(3);
			GPIOG->LCKR = 0x10000 | mask(3);
			(void) GPIOG->LCKR;
		}
	}
